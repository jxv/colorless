...
program.version('0.0.0').option('-s --src [type]', 'Directory of specs OR JSON containing array of specs').option('-d --dest [type]', 'Directory to generate code').option('-n --name [type]', 'Name of top level source file and directory').option('-l --lang [type]', 'Language of code').option('-m --prefix [type]', 'Prefix or module name').option('-e --side [type]', '\'client\' or \'server\' side code', 'client').option('-v --major [type]', 'Oldest supported major version', '0').option('-a --addon [type]', 'Add-on code for client-side or server-side. May require additional dependencies.').parse(process.argv);
...
const supportedSpecs = (prefix, specForLang, diffs, jsonSpecs) => {
  ...
  return attachTypeSources(specs, dropLowMinors(tyVers)).filter(({ version }) => version.major >= +program.major);
};
...
const typeChanges = diff => ({
  major: R.uniq(R.flatten([diff.removeType.map(name => ({ name, action: 'remove' })), diff.modifyType.map(name => ({ name, action: 'modify' })), diff.modifyWrap.map(name => ({ name, action: 'modify' })), diff.modifyStruct.map(name => ({ name, action: 'modify' })), diff.modifyEnumeration.map(x => ({ name: x.name, action: 'modify' }))])),
  minor: R.uniq(R.flatten([diff.addType.map(name => ({ name, action: 'add' }))]))
});
...
const expandTypes = s => {
  const types = R.values(R.mapObjIndexed((ty, n) => R.merge({ n }, (() => {
    if (typeof ty === 'string') {
      return { w: ty };
    }
    if (Array.isArray(ty)) {
...
const nextTypeVersion = (typeVersion, version, change) => {
  ...
  return {
    version: version,
    types: R.merge(R.reduce(R.dissoc, typeVersion.types, removeTypes), R.mergeAll(R.concat(addTypes, modifyTypes).map(name => ({ [name]: version }))))
  };
};
...
const dropLowMinors = tyVers => {
  ...
  R.reverse(tyVers).forEach(tyVer => {
    if (!!lastTyVer) {
      if (tyVer.version.major === lastTyVer.version.major) {} else {
...
const generate = (program, lang, side, gen) => {
  if (program.lang === lang && program.side === side && program.name && program.name.length && program.dest && program.dest.length && program.src && program.src.length) {
...
(function () {
  generate(program, 'javascript', 'client', generateJavascriptClient) || generate(program, 'haskell', 'server', generateHaskellServer) || generate(program, 'haskell', 'client', generateHaskellClient) || generate(program, 'ruby', 'server', generateRubyServer) || (() => console.log('Bad args'))();
})();