...
const genHandlerMap = specs => {
  ...
  lines.add(specs.map(spec => '    , V' + spec.version.major + '.' + spec.name + '\'Service meta' + spec.version.major + ' m\n'));
  lines.add(['    )\n', '  => (xtra -> C.Hooks m ', isUserDefined(specs[0].meta) ? 'V' + specs[0].version.major + '\.' + specs[0].meta : specs[0].meta, ' meta' + specs[0].version.major + ')\n']);
  lines.add(specs.slice(1).map(({ version, meta }) => '  -> C.Hooks m ' + meta + ' meta' + version.major + '\n'));
  lines.add(['  -> xtra\n', '  -> R.Map C.Major (C.Minor, C.Request -> m (P.Either C.Response C.Response))\n', specs[0].lowercaseName, '\'handlerMap']);
  lines.add(specs.map(({ version, meta }) => ' hooks' + version.major));

  lines.add(' xtra = R.fromList\n');
  lines.add(['    [ (' + specs[0].version.major, ', (', specs[0].version.minor, ', V', specs[0].version.major, '.', specs[0].lowercaseName, '\'handler hooks', specs[0].version.major, ' xtra))\n']);

  lines.add(specs.slice(1).map(spec => '    , (' + spec.version.major + ', (' + spec.version.minor + ', V' + spec.version.major + '.' + spec.lowercaseName + '\'handler hooks' + spec.version.major + ' xtra))\n'));
  lines.add('    ]\n');
  return lines;
};
...
const genPublicSpec = (lowercaseName, specs) => {
  ...
  specs.slice(1).forEach(spec => lines.add(['  , V', spec.version.major, '\.', specs[0].lowercaseName, '\'spec\n']));
  lines.add('  ]\n');
  return lines;
};
...
const latest = (specs, addons) => {
  ...
  specs.forEach(spec => lines.add(genVersionImports(spec.module, spec.name, spec.lowercaseName, spec.version.major, mkExportTypes(spec))));
