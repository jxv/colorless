...
const genApi = (name, calls) => {
  var lines = new Lines();
  lines.add(['\n', '-- Api\n', 'data ', name, '\'Api\n']);
  lines.add(['  = ', name, '\'Api\'', calls[0].name, calls[0].filled ? ' ' + calls[0].name : '', '\n']);
  calls.slice(1).forEach(call => lines.add(['  | ', name, '\'Api\'', call.name, call.filled ? ' ' + call.name : '', '\n']));
...
const mkServiceCalls = s => {
  return [].concat(s.hollow.map(x => R.merge(x, { hollow: true }))).concat(s.wrap).concat(s.struct).concat(s.enumeration).filter(isFunc);
};

const genService = (name, lowercaseName, calls) => {
  var lines = new Lines(['\n', '-- Service\n', 'class P.Monad m => ', name, '\'Service meta m where\n']);
  calls.forEach(call => lines.add(['  ', lowercaseName, '\'', call.name, ' :: meta ->', !call.hollow ? ' ' + call.name + ' ->' : '', ' m ', call.output, '\n']));

  lines.add(['\n', 'instance ', name, '\'Service meta m => ', name, '\'Service meta (M.ExceptT C.Response m) where\n']);
  calls.forEach(call => lines.add(['  ', lowercaseName, '\'', call.name, ' _meta = M.lift ', !call.hollow ? ' P.. ' : ' P.$ ', lowercaseName, '\'', call.name, ' _meta\n']));
...
const genApiParser = (name, lowercaseName, calls) => {
  ...
  if (calls.hollow.length) {
    ...
    lines.add(['     [ ("', calls.hollow[0].label, '", ', name, '\'Api\'', calls.hollow[0].name, ')\n']);
    calls.hollow.slice(1).forEach(hollow => lines.add(['     , ("', hollow.label, '", ', name, '\'Api\'', hollow.name, ')\n']));
    lines.add('     ]\n');
  } else {
  ...
  if (calls.struct.length) {
    ...
    lines.add(['     [ ("', calls.struct[0].label, '", v ', name, '\'Api\'', calls.struct[0].name, ')\n']);
    calls.struct.slice(1).forEach(struct => lines.add(['     , ("', struct.label, '", v ', name, '\'Api\'', struct.name, ')\n']));
    lines.add('     ]\n');
  } else {
  ...
  if (calls.enumeration.length) {
    ...
    lines.add(['     [ ("', calls.enumeration[0].label, '", v ', name, '\'Api\'', calls.enumeration[0].name, ')\n']);
    calls.enumeration.slice(1).forEach(enumeration => lines.add(['     , ("', enumeration.label, '", v ', name, '\'Api\'', enumeration.name, ')\n']));
    lines.add('     ]\n');
  } else {
  ...
  if (calls.wrap.length) {
    ...
    lines.add(['     [ ("', calls.wrap[0].label, '", v ', name, '\'Api\'', calls.wrap[0].name, ')\n']);
    calls.wrap.slice(1).forEach(wrap => lines.add(['     , ("', wrap.label, '", v ', name, '\'Api\'', wrap.name, ')\n']));
    lines.add('     ]\n');
  } else {
...
const mkApiLookupPairs = s => {
  return {
    ...
    filled: [].concat(s.wrap).concat(s.struct).concat(s.enumeration).filter(isFunc)
  };
};
...
const mkApiCalls = s => {
  ...
  return [].concat(s.hollow.filter(isFunc).map(notFilled)).concat(s.struct.filter(isFunc).map(filled)).concat(s.enumeration.filter(isFunc).map(filled)).concat(s.wrap.filter(isFunc).map(filled));
};
