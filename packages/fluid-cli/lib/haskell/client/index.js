...
const genModule = (name, lowercaseName, prefix, version, types, values) => {
  ...
  types.forEach(type => lines.add(['  , ', type, '(..)\n']));
...
const mkExportValues = s => {
  var calls = [].concat(s.hollow).concat(s.wrap).concat(s.struct).concat(s.enumeration).filter(isFunc).map(x => s.lowercaseName + '\'' + x.name);

  var exprMk = [].concat(s.struct).concat(s.wrap).map(x => x.lowercaseName + '\'Mk');
  s.enumeration.forEach(({ lowercaseName, enumerals }) => enumerals.forEach(({ tag }) => exprMk.push(lowercaseName + '\'' + tag + '\'Mk')));

  var expr = [].concat(s.struct).concat(s.wrap).map(x => x.lowercaseName + '\'');
  expr = expr.concat(s.enumeration.map(({ lowercaseName }) => lowercaseName + '\''));

  var paths = [];
  s.struct.forEach(struct => struct.members.forEach(member => paths.push(struct.lowercaseName + '\'' + member.name)));
  s.enumeration.forEach(({ lowercaseName, enumerals }) => enumerals.forEach(({ tag, members }) => {
    if (members) {
      members.forEach(member => paths.push(lowercaseName + '\'' + tag + '\'' + member.name));
    }
  }));
...
const genWrapExpr = ({ name, lowercaseName, type }) => {
  ...
  lines.add([lowercaseName, '\'Mk = C.unsafeWrapExpr\n']);
...
const genStructToAst = ({ name, label, members }) => {
  ...
  lines.add('    }');
  lines.add([' = Ast.Ast\'Struct P.. Ast.Struct P.$ R.fromList\n', '    [ ("', members[0].label, '", Ast.toAst ', memberName(name, members[0].name), ')\n']);
  members.slice(1).forEach(member => lines.add(['    , ("', member.label, '", Ast.toAst ', memberName(name, member.name), ')\n']));
  lines.add('    ]\n');
...
const genStructExpr = ({ name, lowercaseName, members }) => {
  ...
  members.slice(1).forEach(member => lines.add([' -> ', member.type]));
  lines.add([' -> ', name, ')\n']);

  lines.add([lowercaseName, '\'Mk = C.unsafeStructExpr ["', members[0].label, '"']);
  members.slice(1).forEach(member => lines.add([', "', member.label, '"']));
  lines.add(']\n');
...
const genEnumerationMatch = ({ name, lowercaseName, enumerals }) => {
  ...
  lines.add([' = C.unsafeExpr P.$ Ast.Ast\'Match P.$ Ast.Match (Ast.toAst _enumeral)\n', '  [ ']);
...
const genEnumerationToAst = ({ name, enumerals }) => {
  ...
  enumerals.forEach(enumeral => {
    if (!enumeral.members) {
      lines.add(['    ', name, '\'', enumeral.tag, ' -> Ast.Ast\'Enumeral P.$ Ast.Enumeral "', enumeral.label, '" P.Nothing\n']);
    } else {
      ...
      lines.add(['      } -> Ast.Ast\'Enumeral P.$ Ast.Enumeral "', enumeral.label, '" P.$ P.Just P.$ R.fromList\n']);
      lines.add(['      [ ("', enumeral.members[0].label, '", Ast.toAst ', memberName(name + '\'' + enumeral.tag, enumeral.members[0].name), ')\n']);
      enumeral.members.slice(1).forEach(member => lines.add(['      , ("', member.label, '", Ast.toAst ', memberName(name + '\'' + enumeral.tag, member.name), ')\n']));
      lines.add('      ]\n');
    }
  });
...
const genEnumeralExpr = ({ name, lowercaseName, enumerals }) => {
  ...
  enumerals.forEach(enumeral => {
    if (!enumeral.members) {
      ...
      enumeral.members.slice(1).forEach(member => lines.add([' -> ', member.type]));
      lines.add([' -> ', name, ')\n']);

      lines.add([lowercaseName, '\'', enumeral.tag, '\'Mk = C.unsafeEnumeralExpr "', enumeral.label, '" ["', enumeral.members[0].label, '"']);
      enumeral.members.slice(1).forEach(member => lines.add([', "', member.label, '"']));
      lines.add(']\n');
    }
  });
